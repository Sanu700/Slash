...
import { Search, MoreVertical, Mail, Phone, Calendar, MapPin, Star, Eye, Download } from 'lucide-react';
import {
...
const Providers = () => {
  ...
  const handleStatusUpdate = async (providerId: string, newStatus: DatabaseProvider['status']) => {
    try {
  ...
  const handleExportData = () => {
    try {
      // Convert providers data to CSV format
      const headers = ['ID', 'Company Name', 'Email', 'Contact No', 'Location', 'Join Date', 'Status', 'Experiences', 'Rating'];
      const csvData = providers.map(provider => [
        ...
        provider.contact_no,
        provider.location,
        new Date(provider.join_date).toLocaleDateString(),
        provider.status,
  ...
  const getStatusBadge = (status: DatabaseProvider['status']) => {
    const variants: Record<DatabaseProvider['status'], "default" | "secondary" | "destructive" | "outline"> = {
      active: "default",
      inactive: "secondary",
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    <p><span className="text-muted-foreground">Name:</span> {selectedProvider.company_name}</p>
                    <p><span className="text-muted-foreground">Email:</span> {selectedProvider.email}</p>
                    <p><span className="text-muted-foreground">Phone:</span> {selectedProvider.contact_no}</p>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    <p><span className="text-muted-foreground">Member Since:</span> {new Date(selectedProvider.join_date).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
            </div>
